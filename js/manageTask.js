document.addEventListener('DOMContentLoaded', () => {
    ...
    // membuat instance dari objek tasks
    const myTask = new Task();
    // membuat variabel unutk mengambil seluruh task
    const existingTask = myTask.getTask();
        ...
        if(existingTask.length === 0) {
            ...
            itemTask.innerHTML = `
                        ...
                        ${task.isCompleted === false ?
                            `<a href="#" id="completeTask-${task.id}"
                                class="flex gap-[10px] justify-center items-center text-white p-[12px_20px] h-12 font-semibold 
                                bg-gradient-to-b from-[#977FFF] to-[#6F4FFF] rounded-full w-full 
                                border border-taskia-background-grey">Complete</a>`
                        :
            ...
            `;
                ...
                itemTask.querySelector(`#completeTask-${task.id}`).addEventListener('click', function(event) {
                    ...
                    const updateTasks = myTask.getTask();
